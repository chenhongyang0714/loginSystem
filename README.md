...
3. 通过弹出对话框提示登录是否成功(只有在鼠标点击登录时生效)
4. 点击Exit退出登录程序
...
### 注册账号
1. 密码不可以为空
2. 通过匹配用户名进行查询要注册的用户是否已经存在(没有GUI对话框的提示)

### BUG
1. 通过弹出对话框提示登录是否成功(只有在鼠标点击登录时生效)
2. 教师、学生 可同时选择
3. 修改密码时当用户不存在(没有GUI对话框的提示) 
4. 通过匹配用户名进行查询要注册的用户是否已经存在(没有GUI对话框的提示)
